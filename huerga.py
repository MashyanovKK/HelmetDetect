...
class Detector:
    ...
    def detection(self, frame):
        ...
        for data in detected:
                    ...
                    results[class_id].append([xmin, ymin, xmax, ymax])
                    # cv2.rectangle(frame, (xmin,ymin), (xmax, ymax), (0, 255, 0), 2)
                    # cv2.putText(frame, f'{0}', (xmin,ymin), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), class_id)
    ...
    def analyze(self, frame, detections):
        ...
        if not len(detections[1]):
            for i in detections[0]:
                ...
                cv2.putText(frame, f'{0}', (i[0], i[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, red, 2)
        else:
            for person in detections[0]:
                ...
                hats = sorted(hats, key = lambda x : x[4],reverse=False)
                hat = hats[0]
                hatCenter = (hat[0]+(hat[2] - hat[0])/2, hat[1]+(hat[3] - hat[1])/2)
